...
function Profile() {
    ...
    return (
        ...
        >
            <main className='px-4 md:px-8 lg:px-12 py-8 bg-[#F2F2F2] min-h-screen'>
                {/* Header Section */}
                <motion.div
                ...
                >
                    <div className='flex flex-col md:flex-row items-center gap-6 w-full lg:w-auto text-center md:text-left'>
                        <div className="relative shrink-0">
                            <div className="w-32 h-32 md:w-40 md:h-40 lg:w-44 lg:h-44 rounded-full overflow-hidden border-4 border-white shadow-xl bg-[#176D69]/10 flex items-center justify-center transition-transform duration-500 hover:scale-105 relative">
                                {(() => {
                                    const userFromStorage = JSON.parse(localStorage.getItem('user') || '{}');
                                    const rawImage = liveImageUrl || data.image || userFromStorage.imageUrl || localStorage.getItem('profileImageUrl');
                                    if (!rawImage || rawImage === 'null' || rawImage === 'undefined' || imageError) {
                                        return <User className="text-[#176D69]" size={64} strokeWidth={1.5} />;
                                    }

                                    const resolvedImage = rawImage.startsWith('http') ? rawImage : `/proxy/${rawImage.replace(/^\//, '')}`;
                        ...
                        <div className='text-[#093332]'>
                            ...
                            <span className='block mb-3 opacity-80'>{data.specialization}</span>
                    ...
                    <div className='flex flex-col justify-center items-center lg:items-end gap-3 mt-6 lg:mt-0'>
                        <a href={data.website} target="_blank" rel="noopener noreferrer" className='flex items-center gap-1 text-[#564FFD] hover:underline transition-all'>
                ...
                <div className="max-w-6xl mx-auto min-h-screen font-sans">
                    {/* About section */}
                    <motion.div {...fadeInUp} className='pb-10'>
                        <h2 className="text-2xl font-bold mb-4 text-[#093332]">ABOUT ME</h2>
                        <p className="text-[#176D69] leading-relaxed max-w-4xl">
                            {data.bio || "EduNEXA Instructor focusing on delivering high-quality educational content and helping students achieve their professional goals."}
                        </p>
                    </motion.div>
