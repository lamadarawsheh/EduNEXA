    ...
    const handleSubmit = async (e) => {
        ...
        if (validate()) {
            ...
            try {
                ...
                // Show Dynamic Success Popup with Username
                setPopup({
                    show: true,
                ...
                // Extract role from the roles array (e.g., "Instructor", "Admin", "Student")
                const roleFromResponse = (data.roles && data.roles.length > 0) ? data.roles[0] : 'Student';
                const userRole = roleFromResponse.toLowerCase();

                // Wait briefly so user can see the success message
                setTimeout(() => {
                    if (userRole === 'admin') {
                        navigate('/admin');
