...
export default function CourseContent({ courseData, currentLessonTitle, onLessonClick }) {
  const [openWeek, setOpenWeek] = useState(courseData?.[0]?.id || null);
    ...
    <div className="w-full bg-white border border-[#e2e8f0] rounded-sm shadow-sm font-sans">
      ...
      <div className="w-full">
        {courseData?.map((section, index) => {
          const isOpen = openWeek === section.id;
          
          return (
            <div key={section.id || index} className="border-b border-[#176D69] last:border-b-0">
              
              <button 
                onClick={() => setOpenWeek(isOpen ? null : section.id)}
                    ...
                    return (
                      <div 
                        ...
                        className={`group flex items-center gap-4 px-6 py-4 cursor-pointer transition-all border-y border-transparent ${
                          isCurrent ? 'bg-[#e7f6f5] border-[#b2dfdb]' : 'hover:bg-emerald-50/30'
                        }`}
                      >
                        <div className={`w-[20px] h-[20px] rounded-sm border flex items-center justify-center flex-shrink-0 ${
                          lesson.isCompleted ? 'bg-[#08332e] border-[#08332e]' : 'border-gray-300 bg-white'
                        }`}>
